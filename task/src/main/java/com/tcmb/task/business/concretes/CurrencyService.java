package com.tcmb.task.business.concretes;

import com.tcmb.task.business.abstracts.ICurrencyService;
import com.tcmb.task.core.adapters.ITcmbServiceAdapter;
...
import com.tcmb.task.dataAccess.abstracts.CurrencyDao;
    ...
    public CompletableFuture<DataResult<List<Currency>>> getAll(String url, LocalDate date) {
        ...
        try {
            if (!isExchangeRateExist(date).isSuccess()) {
                CompletableFuture<DataResult<List<Currency>>> result = iTcmbServiceAdapter.getExchangeRate(url);
